...
export default function DashboardPage() {
  ...
  return (
    <ErrorBoundary>
      <div>
        <h1 className="text-2xl font-semibold text-gray-100 mb-6 font-heading">
          Dashboard
        </h1>
        ...
        <Card>
          <div className="px-6 py-4 border-b border-white/[0.06]">
            <h2 className="text-lg font-semibold text-gray-100 font-heading">
              Your Coachees
            </h2>
          </div>
...
function StatCard({
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = `0 0 20px ${color}20, 0 0 40px ${color}10`;
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = '';
      }}
    >
      <div className="absolute left-0 top-0 bottom-0 w-1 transition-all duration-300 group-hover:w-1.5" style={{ background: color }} />
      <div className="flex items-start justify-between mb-2">
        <div className="text-[10px] uppercase tracking-widest text-gray-500 font-semibold">{label}</div>
        <div className="p-1.5 rounded-lg bg-white/[0.04] transition-colors duration-300 group-hover:bg-white/[0.08]" style={{ color }}>
          {icon}
        </div>
      </div>
